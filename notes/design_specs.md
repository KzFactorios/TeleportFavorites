...
- `core/map_tag/` – Class definitions and methods for managing map_tags
- `core/control/` – Lifecycle, event, and utility modules.
- `core/patterns/` – Class definitions and methods for managing deesign patterns
...
- Only players who have created a chart_tag or map_tag (or where the last_user information is blank or nil) may edit a map_tag and associated chart_tag. If a players selects a map_tag and is allowed to make changes, then the associated chart_tag's last user sohuld be updated to the editing player's player.name. Chart_tag.last_user should always use the player's name and not the index or player object.
- if a player leaves the game (or is kicked out, etc):
    -- for all storage.players[player_index]surfaces[].favorites, examine their favorites and update any matching map_tags by removing the player's index from the faved_by_players
    -- if now, the related map_tag is owned by the player (chart_tag.last_user == player.name) has no faved_by_players, then delete the map_tag (and associated chart_tag)
    -- next, check all surfaces for any map_tags created by the player. for each map_tag, ensure the player's index in faved_by_players is removed (if present). And if the the faved_by_players is empty after this check, then delete the map_tag. ie: if another player has favorited the position, then do not delete. If the map_tag is not deleted, reset the last_user to nil or ""
