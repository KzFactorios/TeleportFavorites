...
---@param tbl table
---@param key_func function Function that takes (value, key) and returns group key
---@return table grouped_table
function CollectionUtils.group_by(tbl, key_func)
  if type(tbl) ~= "table" or type(key_func) ~= "function" then return {} end
  local groups = {}
  for k, v in pairs(tbl) do
    local group_key = key_func(v, k)
    if not groups[group_key] then
      groups[group_key] = {}
    end
    table.insert(groups[group_key], v)
  end
  return groups
end
...
---@param tbl table
---@param predicate_func function Function that takes (value, key) and returns boolean
---@return boolean any_match
function CollectionUtils.any(tbl, predicate_func)
  if type(tbl) ~= "table" or type(predicate_func) ~= "function" then return false end
  for k, v in pairs(tbl) do
    if predicate_func(v, k) then
      return true
    end
...
function CollectionUtils.all(tbl, predicate_func)
  if type(tbl) ~= "table" or type(predicate_func) ~= "function" then return true end
  for k, v in pairs(tbl) do
    if not predicate_func(v, k) then
      return false
    end
  end
  return true
...
--- Take first N elements from a table
---@param tbl table
---@param count number
---@return table first_n_elements
function CollectionUtils.take(tbl, count)
  if type(tbl) ~= "table" or type(count) ~= "number" then return {} end
  local result = {}
  local i = 0
  for k, v in pairs(tbl) do
...
--- Skip first N elements from a table
---@param tbl table
---@param count number
---@return table remaining_elements
function CollectionUtils.skip(tbl, count)
  if type(tbl) ~= "table" or type(count) ~= "number" then return CollectionUtils.shallow_copy(tbl) end
  local result = {}
  local i = 0
  for k, v in pairs(tbl) do
    if i >= count then
