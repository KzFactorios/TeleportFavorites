...
function Positionhelpers.position_can_be_tagged(player, map_position)
  if not (player and player.force and player.surface and player.force.is_chunk_charted) then return false end  local chunk = { x = math.floor(map_position.x / 32), y = math.floor(map_position.y / 32) }
  if not player.force:is_chunk_charted(player.surface, chunk) then
    GameHelpers.player_print(player, "[TeleportFavorites] You are trying to create a tag in uncharted territory: " ..
