...
local function relocate_tag_if_on_water(player, chart_tag)
  ...
  local tag = Cache.get_tag_by_gps(player, old_gps)
  ...
  local new_chart_tag = ChartTagUtils.safe_add_chart_tag(force, surface, chart_tag_spec, nil)
    ...
    for _, player_index in ipairs(tag.faved_by_players) do
      ...
      if player and player.valid then
        GameHelpers.player_print(player,
          RichTextFormatter.tag_relocated_notification(new_chart_tag, old_position, new_position))
      end
    end
  end
...
local function process_changed_tiles(player, tiles)
  if not tiles or #tiles == 0 or not player or not player.valid then return end
  ...
  for _, chart_tag in pairs(chart_tags) do
    if chart_tag and chart_tag.valid then
      ...
      if changed_positions[pos_key] and is_position_on_water(surface, pos) then
        relocate_tag_if_on_water(player, chart_tag)
      end
    end
  end
end
