...
GPS.gps_from_map_position = gps_parser.gps_from_map_position
GPS.map_position_from_gps = gps_parser.map_position_from_gps
GPS.get_surface_index = gps_parser.get_surface_index_from_gps
...
function GPS.coords_string_from_gps(gps)
  ...
  local parsed = gps_parser.parse_gps_string(gps)
...
function GPS.gps_to_gps_tag(gps)
  local parsed = gps_parser.parse_gps_string(gps)
  return parsed and string.format("[gps=%d,%d,%d]", parsed.x, parsed.y, parsed.s) or nil
end
...
--- Normalize landing position using Cache - breaks circular dependency by lazy loading
---@param player LuaPlayer
---@param gps string
---@param Cache table
---@return MapPosition|nil, table|nil, LuaCustomChartTag|nil, table|nil
function GPS.normalize_landing_position_with_cache(player, gps, Cache)
  -- Lazy load gps_helpers to avoid circular dependency at module load time
  local gps_helpers = require("core.utils.gps_helpers")
  return gps_helpers.normalize_landing_position_with_cache(player, gps, Cache)
end
