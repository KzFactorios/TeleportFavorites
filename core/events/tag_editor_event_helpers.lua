...
--- Find or create tag data for tag editor based on cursor position
---@param player LuaPlayer Player opening tag editor
---@param cursor_position table Cursor position where tag editor was opened
---@param surface_index number Surface index
---@return table tag_data Tag editor data structure
function TagEditorEventHelpers.find_or_create_tag_data(player, cursor_position, surface_index)
  ...
  -- Search for nearby chart tags if no exact tag match
  if not nrm_tag then
    nrm_chart_tag = TagEditorEventHelpers.find_nearby_chart_tag(normalized_pos, surface_index, click_radius)
  ...
  if not nrm_tag and not nrm_chart_tag then
    tag_gps = TagEditorEventHelpers.create_temp_tag_gps(normalized_pos, player, surface_index)
  end
  
  -- Finalize GPS and chart tag references
  if nrm_tag then
    tag_gps = nrm_tag.gps
