...
describe("PlayerFavorites uncovered logic", function()
  ...
  assert.is_table(fav2) -- Should return the existing favorite
  assert.is_nil(err2)   -- No error, just returns existing
  ...
  -- Implementation allows removal even if locked
  assert.is_true(ok2)
  assert.is_nil(err2)
  end)
  ...
  it("should count available slots with some filled", function()
    ...
    local pf = PlayerFavorites.new(player)
    pf:add_favorite("gps1")
    assert.equals(pf:available_slots(), 9)
  end)
  ...
  it("should add, get, move, remove, and lock favorites", function()
    ...
    -- Remove favorite (should succeed even if locked)
    local ok3, rem_err = pf:remove_favorite("gps1")
    assert.is_true(ok3)
    assert.is_nil(rem_err)
  ...
  it("should count available slots when all are blank", function()
    local player = { index = 1, valid = true, surface = { index = 1 } }
    local pf = PlayerFavorites.new(player)
  assert.equals(pf:available_slots(), 10)
  end)
end)
