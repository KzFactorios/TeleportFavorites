...
describe("PlayerFavorites", function()
    before_each(function()
        ...
        -- Force clear module cache and reload PlayerFavorites
        package.loaded["core.favorite.player_favorites"] = nil
        
        -- Ensure all dependencies are available
        if not _G.Constants then
        ...
        local success, PlayerFavorites = pcall(require, "core.favorite.player_favorites")
        if not success then
            error("Failed to load PlayerFavorites module: " .. tostring(PlayerFavorites))
        end
        
        if not PlayerFavorites or type(PlayerFavorites) ~= "table" then
            error("PlayerFavorites module did not return a table: " .. type(PlayerFavorites))
        end
        
        if type(PlayerFavorites.new) ~= "function" then
            error("PlayerFavorites.new is not a function: " .. type(PlayerFavorites.new))
        end
        
        _G.PlayerFavorites = PlayerFavorites
    ...
    it("should synchronize tag faved_by_players and favorites across multiplayer add/move/delete", function()
        -- Debug: Check if PlayerFavorites was loaded properly
        if not _G.PlayerFavorites then
            error("_G.PlayerFavorites is nil - before_each did not run or failed to load module")
        end
        if type(_G.PlayerFavorites.new) ~= "function" then
            error("_G.PlayerFavorites.new is not a function: " .. type(_G.PlayerFavorites.new))
        end
